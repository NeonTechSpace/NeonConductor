...
export function toBranchFailureMessage(reason: string): string {
    ...
    if (reason === 'workspace_required') {
        return 'Branch workflows are only available for workspace-bound agent and orchestrator sessions.';
    }
    if (reason === 'branch_workflow_not_found') {
        return 'The selected branch workflow no longer exists in this workspace.';
    }
    if (reason === 'branch_workflow_disabled') {
