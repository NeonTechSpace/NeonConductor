import type { CustomModeEditorDraft } from '@/web/components/settings/modesSettings/modesInstructionsControllerShared';
import {
...
export function MetadataSummary(input: { item: MetadataSummaryItem }) {
    return (
...
export function PreparedContextProfileDefaultsCard(input: {
            ...
            <PreparedContextMatrixScaffold
                ...
                <div className='overflow-x-auto'>
                    <table className='min-w-full border-separate border-spacing-y-2 text-sm'>
                        <thead>
                            <tr className='text-muted-foreground text-left text-xs tracking-[0.12em] uppercase'>
                                <th className='pr-4 pb-1 font-semibold'>Layer</th>
                                {preparedContextInjectionCheckpointOrder.map((checkpoint) => (
                                    <th key={checkpoint} className='pr-4 pb-1 font-semibold'>
...
export function TopLevelPromptSection(input: { title: string; description: string; children: ReactNode }) {
    return (
        <div className='space-y-3'>
            <div className='space-y-1'>
            ...
            <div className='flex flex-wrap items-start justify-between gap-3'>
                ...
                <div className='flex flex-wrap gap-2'>
                    ...
                    <Button
                        ...
                        size='sm'
                        disabled={input.editor.isSaving}
                        onClick={() => void input.editor.save()}>
            ...
            {draft.kind !== 'edit' ? (
                <label className='space-y-2'>
                    ...
                    <textarea
                        ...
                        onChange={(event) => {
                            input.editor.setField('sourceText', event.target.value);
                        }}
                        className='border-border bg-background min-h-28 w-full rounded-2xl border px-4 py-3 text-sm leading-6'
                        spellCheck={false}
                    ...
                    <p className='text-muted-foreground text-xs leading-5'>
                        Optional provenance for a reviewed draft. Pasted content stays in the draft until you apply or
                        discard it.
                    </p>
                </label>
            ) : null}
                ...
                <div className='border-border/70 bg-background/60 space-y-3 rounded-2xl border px-4 py-4'>
                    ...
                    <label className='flex items-start gap-3 text-sm'>
                        <input
                            ...
                            onChange={(event) => {
                                input.editor.setDraftOverwriteConfirmed(event.target.checked);
                            }}
                        />
                        <span className='text-muted-foreground leading-6'>
                            Allow overwrite if applying this draft would replace an existing file-backed mode with the
                            same tab and key.
                        </span>
                    </label>
                    <Button
            ...
            {draft.kind === 'edit' ? (
                <div className='border-destructive/25 bg-destructive/5 space-y-3 rounded-2xl border px-4 py-4'>
                    ...
                    <label className='flex items-start gap-3 text-sm'>
                        ...
                        />
                        <span className='text-muted-foreground leading-6'>
                            I understand this will delete the file-backed custom mode and fall back to normal resolution
                            afterward.
                        </span>
                    </label>
                    <Button
