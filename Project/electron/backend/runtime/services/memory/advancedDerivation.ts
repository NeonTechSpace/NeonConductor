...
import { buildMemoryGraphEdges } from '@/app/backend/runtime/services/memory/memoryGraphDerivation';
...
function buildTemporalResolutionMaps(
    ...
    for (const [groupKey, groupedMemories] of memoriesByGroupKey.entries()) {
        ...
        if (conflictingCurrentMemoryIds.length > 0) {
            ...
            const [activeMemoryId] = activeMemoryIds;
            if (activeMemoryId) {
                currentTruthMemoryIdByGroupKey.set(groupKey, activeMemoryId);
            }
        }
...
function mapDerivedSummary(input: {
    ...
    const conflictingCurrentMemoryIds = subjectFacts
        ...
        .map((fact) => fact.sourceMemoryId);
    const incomingRevisionReason = input.incomingRevisionReasonByMemoryId.get(input.memoryId);
    const outgoingRevisionReason = input.outgoingRevisionReasonByMemoryId.get(input.memoryId);
    ...
    return {
        ...
        ...(incomingRevisionReason ? { incomingRevisionReason } : {}),
        ...(outgoingRevisionReason ? { outgoingRevisionReason } : {}),
        ...(memory
            ? {
...
export class AdvancedMemoryDerivationService {
    ...
    async rebuildProfile(profileId: string): Promise<OperationalResult<{ memoryCount: number }>> {
        ...
        const graphEdges = buildMemoryGraphEdges({
            profileId,
            memories,
            subjectKeyByMemoryId: resolutionMaps.subjectKeyByMemoryId,
            revisionRecords,
            evidenceByMemoryId,
        });
